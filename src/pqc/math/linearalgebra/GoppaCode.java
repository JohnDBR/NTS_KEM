...
public final class GoppaCode
{
    ...
    /**
     * This class is a container for two instances of a and h. Those are the 
     * arrays to create the H check matrix. Also H is contained.
     *
     * @see GF2Matrix
     * @see PermutationCustom
     */
    public static class HCheck
    {
        ...
        private GF2Matrix hCheck;

        /**
         * Construct a new {@link MatrixSet} container with the given
         * parameters.
         *
         * @param a       Array a to generate H matrix
         *                πp*(a') = (ap_0, ap_1, ... ,ap_n−1); a' = (B[0], B[1], 
         *                B[2], ..., B[n−2], B[n−1])
         * @param h       Array h to generate H matrix
         *                πp*(h') = (hp_0, hp_1, ... , h_pn−1); h' = (G(B[0]), 
         *                G(B[1]), ..., G(B[n−1]))
         * @param hCheck  The H matrix produced by a and h
         */
        public HCheck(int[] a, int[] h, GF2Matrix hCheck)
        {
            this.a = a;
            this.h = h;
            this.hCheck = hCheck;
        ...
        /**
         * @return the H matrix produced by a and h
         */
        public GF2Matrix getHCheck()
        {
            return hCheck;
        }
    }
                                                       ...
                                                       PolynomialGF2mSmallM gp)
    {
     ...
     /**
     * Construct the check matrix of a Goppa code in canonical form from the
     * irreducible Goppa polynomial over the finite field with the NTS_KEM 
     * specifications
     * <tt>GF(2<sup>m</sup>)</tt>.
     *
     * @param field the finite field
                                                       ...
                                                       PermutationCustom p)
    {
        ...
        int n = 1 << m;
        int t = gp.getDegree();
        int l = NTS_KEM_Parameters.DEFAULT_L;
        int k = n - m * t;
        ...
        for (int j = 0; j < n; j++)
        {
            // here j is used as index and as element of field GF(2^m)
            a[j] = j;
            int g_j = gp.evaluateAt(j);
            int ig_j = field.inverse(g_j);
            h[j] = field.mult(ig_j, ig_j);
        }
        
        // permute a and h arrays
        a = p.permutateVector(a);
        h = p.permutateVector(h);
        
        // split a and h arrays (a_b | a_c) and (h_b | h_c)
        int[] aSplit = new int[l + n - k];
        int[] hSplit = new int[l + n - k];
        int pos = 0;
        for (int i = k - l; i < k; i++) {
            aSplit[pos] = a[i];
            hSplit[pos] = h[i];
        ...
        for (int i = 0; i < t; i++)
        {
            for (int j = 0; j < n; j++)
            {
                hArray[i][j] =  field.add(hArray[i][j], field.mult(h[j], 
                        a[j]^i));
            }
        }
        ...
        return new HCheck(aSplit, hSplit, new GF2Matrix(n, result));
    }
